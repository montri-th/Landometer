...
>
> Browser-tab delivery SHOULD use a stable same-origin URL for the approved compact asset and MAY add a content-derived cache revision without changing image bytes. Standalone snapshots SHOULD retain that stable URL when a data-URL favicon is not interoperable across supported browsers. The manifest and Build Card MUST bind the URL, cache revision, original asset path, byte count, and SHA-256; cache busting MUST NOT substitute another image or create a new identity variant.

> The browser-tab icon MUST identify the owning website or product at tab size. Do not substitute a generic initial, unrelated page imagery, or a convenient content thumbnail. A shared Landometer surface SHOULD use the separately approved shared compact symbol; a product-specific surface MUST use its own separately approved product asset and MUST NOT silently inherit or redefine shared Landometer identity.
...
> A cross-device color comparison, visual approval, or handoff MUST first prove that every compared channel resolves the same declared Color Set and UI artifact build. Color identity and UI-build identity are separate. A governed color, gradient, token-source, or analytical scale-source change MUST mint a new Color Set ID. A UI-only change MUST preserve the Color Set ID and mint a new append-only artifact-build ID and immutable filename.
>
> The hosted root/latest alias, latest standalone alias, current immutable UI artifact, Complete Color Atlas, Site Manifest, and Build Card MUST carry one registry/build relationship: Design System version, Color Set ID, artifact-build ID, token-registry path/version/SHA-256, scale-registry path/version/SHA-256, and the expected SHA-256 for each delivered artifact. Channel-specific packaging MAY have a different file hash only when the manifest and Build Card record the derivation and bind it to that same Color Set and artifact build.
>
> Every immutable Color Set baseline and immutable UI artifact path is append-only. Its bytes and SHA-256 MUST remain stable for that identity. A mutable hosted root or latest standalone is a convenience alias, not an approval authority. When delivery uses a non-zero cache lifetime, different devices MAY temporarily receive different latest aliases. A mismatched registry, artifact build, or expected hash is build/cache skew; stop the visual comparison and resolve delivery parity before diagnosing rendering.
>
> A release build SHOULD rebuild derived artifacts from source in a non-writing check mode and compare the committed bytes. It MUST validate every immutable target before replacing a mutable alias, and SHOULD use atomic writes so a failed immutability check cannot leave a partial release. Automated QA evidence MUST name the artifact-build ID, exact artifact hashes, tested state matrix, and remaining manual gates. Post-deploy verification MUST compare the deployed build and hashes after deployment completes; a version marker alone is insufficient.
>
