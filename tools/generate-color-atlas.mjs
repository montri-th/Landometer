...
const COLOR_DELIVERY_SOURCE = path.join(
  ...
  "assets",
  "data",
  "color-delivery.v0.9.1.json",
);
const RETAINED_COLOR_DELIVERY_SOURCE = path.join(
  PROJECT_ROOT,
  "deployment",
  "assets",
  "data",
...
const PREVIEW_START_MARKER = "<!-- COLOR_ATLAS_PREVIEW_START -->";
const PREVIEW_END_MARKER = "<!-- COLOR_ATLAS_PREVIEW_END -->";
...
function validateColorDelivery(registry, scaleText, tokenText) {
  ...
  assert(
    /^ui-\d{8}-\d{2}$/.test(registry?.meta?.currentArtifactBuild?.id ?? "") &&
      new RegExp(`^landometer-design-system-v0\\.9\\.1-standalone\\.${registry.meta.id}\\.ui-\\d{8}-\\d{2}\\.html$`).test(
...
function validateIdentityColors(tokenSource) {
  const tokenValues = {
    "brand.blue": tokenSource?.brand?.blue,
    "brand.beige": tokenSource?.brand?.beige,
    "dark.brand.beige": tokenSource?.brand?.darkBeige,
    "energy.sky": tokenSource?.energy?.sky,
    "energy.mint": tokenSource?.energy?.mint,
    "energy.coral": tokenSource?.energy?.coral,
    "energy.yellow": tokenSource?.energy?.yellow,
  };
  for (const record of identityColors) {
    assert(
      tokenValues[record.id] === record.value,
      `identity color ${record.id} must match tokens.json`,
    );
  }
  assert(
    Object.keys(tokenValues).length === identityColors.length,
    "identity preview token coverage drift",
  );
}

function buildAtlasPreview(colorDelivery) {
  const swatches = identityColors
    .map(
      (record) =>
        `<span class="v091-atlas-ribbon__swatch" role="listitem" data-atlas-token="${escapeHtml(record.id)}" data-atlas-value="${escapeHtml(record.value)}" style="--swatch:${escapeHtml(record.value)};background:${escapeHtml(record.value)}" title="${escapeHtml(`${record.id} · ${record.value}`)}" aria-label="${escapeHtml(`${record.id}: ${record.value}`)}"></span>`,
    )
    .join("");

  return `<!-- Generated by tools/generate-color-atlas.mjs from the identityColors registry. Do not hand-edit. -->
          <div class="v091-atlas-ribbon" aria-label="ตัวอย่างสีอัตลักษณ์และ Energy จาก Color Atlas" data-l10n-aria-th="ตัวอย่างสีอัตลักษณ์และ Energy จาก Color Atlas" data-l10n-aria-en="Identity and Energy samples from the Color Atlas" data-atlas-preview-family="identity-energy" data-color-registry="${escapeHtml(colorDelivery.meta.id)}" role="list">${swatches}</div>`;
}
          ...
          <article class="atlas-scale-record" data-atlas-scale="${escapeHtml(record.scaleId)}" data-atlas-theme="${escapeHtml(record.theme)}" data-atlas-kind="${escapeHtml(record.kind)}">
            ...
            <figure class="atlas-lut-figure">
              ...
              <div
                class="atlas-lut"
                role="img"
                aria-label="${escapeHtml(`${labelTh} ตารางสี 41 ขั้น ธีม${record.theme === "dark" ? "มืด" : "สว่าง"} จาก ${record.lut[0]} ถึง ${record.lut[40]}`)}"
                data-l10n-aria-th="${escapeHtml(`${labelTh} ตารางสี 41 ขั้น ธีม${record.theme === "dark" ? "มืด" : "สว่าง"} จาก ${record.lut[0]} ถึง ${record.lut[40]}`)}"
                data-l10n-aria-en="${escapeHtml(`${labelEn} ${record.theme} 41-stop lookup table from ${record.lut[0]} through ${record.lut[40]}`)}"
              >
                ${renderCells(
                  record.lut,
                  "atlas-lut-cell",
      ...
      >
        <header class="scale-sampler-head">
          ...
          <span
            ...
            data-l10n-aria-th="ธีมที่แสดงอยู่: สว่าง"
            data-l10n-aria-en="Visible theme: light"
        ...
        <div class="scale-sampler-boundary" role="note">
          <strong>ANALYTICAL REFERENCE · NOT PRODUCT EVIDENCE</strong>
          <p>${bilingual(
            "ชุดสีนี้สืบทอดจาก scales.json v0.8.6 เข้า v0.9.1 เพื่อใช้เรียนรู้และตรวจแบบ ไม่ใช่หลักฐานหรือ dataviz package ของผลิตภัณฑ์",
            "These families are carried from scales.json v0.8.6 into v0.9.1 for teaching and review. They are not product evidence or a product dataviz package.",
          )}</p>
        </div>
...
<section class="atlas-root" aria-labelledby="atlas-title" data-color-registry="${escapeHtml(colorDelivery.meta.id)}" data-atlas-version="0.9.1" data-atlas-source-version="${escapeHtml(scaleSource.meta.version)}" data-atlas-records="${scaleSource.scales.length}">
  <header class="atlas-intro">
    ...
    <div class="atlas-boundary atlas-boundary--source" role="note">
      <strong>ANALYTICAL REFERENCE · NOT PRODUCT EVIDENCE</strong>
      <p>${bilingual(
        "LUT ด้านล่างอ่านจาก scales.json ซึ่งเป็น reference fixture ที่สืบทอดเข้า v0.9.1 ใช้เรียนรู้ ตรวจแบบ และเทียบค่าล่วงหน้าได้ แต่ไม่ใช่หลักฐานหรือ dataviz package ของผลิตภัณฑ์ งานวิเคราะห์จริงต้อง bind schema, release, unit และ grain ที่เข้ากันก่อนใช้",
        "The LUTs below come from the scales.json reference fixture carried into v0.9.1. Use them to learn, review, and compare—not as product evidence or a product dataviz package. Real analysis must bind compatible schema, release, unit, and grain before use.",
      )}</p>
    </div>
    <div class="atlas-counts" aria-label="ขอบเขตของ Color Atlas" data-l10n-aria-th="ขอบเขตของ Color Atlas" data-l10n-aria-en="Color Atlas coverage">
...
${logoAssetColors.map(renderLogoCard).join("")}
  ...
  <section class="atlas-family atlas-family--foundation" aria-labelledby="atlas-foundation-title">
    <header class="atlas-family-head">
      ...
      <p>${bilingual(
        "17 คู่ด้านล่างคือ active authoring subset ที่สืบทอดเข้า v0.9.1 เลือกตามบทบาท ไม่เลือกเพราะชอบรหัสสี",
        "These 17 light/dark pairs are the active authoring subset carried into v0.9.1. Choose by role, never by favorite hex.",
      )}</p>
    </header>
...
${mapPairs.map(renderMapPair).join("")}
  ...
  <section class="atlas-family atlas-family--depth" aria-labelledby="atlas-depth-title">
    ...
    <div class="atlas-depth-layout">
      <div class="atlas-opacity-list" aria-label="ทะเบียนค่า opacity มาตรฐาน" data-l10n-aria-th="ทะเบียนค่า opacity มาตรฐาน" data-l10n-aria-en="Canonical opacity registry">
...
${depthRoles.map(renderDepth).join("")}
  ...
  <footer class="atlas-footer">
    ...
    <code>landometer-design-system@0.9.1 · color-srgb-05 retained · REFERENCE</code>
  </footer>
</section>
`;
}
...
async function main() {
  const [scaleText, tokenText, colorDeliveryText, retainedColorDeliveryText] = await Promise.all([
    readFile(SCALE_SOURCE, "utf8"),
    readFile(TOKEN_SOURCE, "utf8"),
    readFile(COLOR_DELIVERY_SOURCE, "utf8"),
    readFile(RETAINED_COLOR_DELIVERY_SOURCE, "utf8"),
  ]);
  const scaleSource = JSON.parse(scaleText);
  const tokenSource = JSON.parse(tokenText);
  const releaseColorDelivery = JSON.parse(colorDeliveryText);
  const retainedColorDelivery = JSON.parse(retainedColorDeliveryText);
  const colorDelivery = {
    ...retainedColorDelivery,
    ...releaseColorDelivery,
    meta: { ...retainedColorDelivery.meta, ...releaseColorDelivery.meta },
    sources: { ...retainedColorDelivery.sources, ...releaseColorDelivery.sources },
    delivery: { ...retainedColorDelivery.delivery, ...releaseColorDelivery.delivery },
  };
  validateScaleSource(scaleSource);
  validateColorDelivery(colorDelivery, scaleText, tokenText);
  validateIdentityColors(tokenSource);

  const fragment = buildFragment(scaleSource, colorDelivery);
  const sampler = buildScaleSampler(scaleSource, colorDelivery);
  const preview = buildAtlasPreview(colorDelivery);
  ...
  assert(
    (preview.match(/class="v091-atlas-ribbon__swatch"/g) ?? []).length ===
      identityColors.length,
    "atlas preview must contain every identity and energy color exactly once",
  );
  for (const record of identityColors) {
    assert(
      preview.includes(`data-atlas-token="${record.id}" data-atlas-value="${record.value}"`),
      `atlas preview is missing ${record.id} ${record.value}`,
    );
  }
  assert(!preview.includes("--series-"), "atlas preview must not borrow categorical colors");
  ...
  if (process.argv.includes("--inject") || process.argv.includes("--check-index")) {
    ...
    const previewSection = inspectMarkedSection(
      indexHtml,
    ...
    if (process.argv.includes("--check-index")) {
      ...
      assert(
        previewSection.embedded === preview.trim(),
        "embedded index.html color atlas preview is not current",
      );
      assert(
  ...
  process.stdout.write(
    `Wrote ${OUTPUT_PATH} (${scaleSource.scales.length} scale records; 738 LUT cells; 9-family sampler with 189 paired class cells; ${identityColors.length}-color identity preview)${process.argv.includes("--inject") ? " and injected deployment/index.html" : ""}\n`,
  );
}
