# Landometer Design System v0.8.9

**Release:** v0.8.9
**Authoring revision:** v0.8.9-r1 — owner-approved harmonized gradient-diversity family integrated 7 August 2026 over the complete v0.8.8-r3 authoring authority
**Manifest version:** 2.0 — unchanged
**Token schema version:** 6 — unchanged
**Build Card schema version:** 0.8.9
**Status:** Owner-approved simplified normative authoring master; the v0.8.9-r1 public-safe machine package, release lock, source fingerprint, rule ledger, schemas, recipes, and preflight MUST be regenerated before package-level v0.8.9 conformance is claimed
**Prepared:** 25 July 2026; v0.8.8 experience clarifications approved 6 August 2026; brand-line authority corrected 7 August 2026; v0.8.9 gradient diversity approved and integrated 7 August 2026
**Supersedes:** Landometer Design System v0.8.8-r3 for human-readable authoring. The frozen v0.8.8 Color Set `color-srgb-01`, its immutable artifacts, and earlier packages remain read-only migration and compatibility evidence. v0.8.9 retains Manifest 2.0 and Token Schema 6, adds fifteen exact gradient-only raw values, seven shared atmosphere recipes, and immutable Color Set `color-srgb-02`; it adds no Profile, Trigger Pack, optional capability, menu, mode, or default-route scene. Package-level validation never certifies an individual implementation: every artifact must still pass its applicable Build Card, asset, evidence, browser, accessibility, privacy, composition, and effect gates.
...
1. current owner amendments dated 21–25 July and 6–7 August 2026 govern the amended outcome: simplify the active source, release unmistakable positive Landometer energy, preserve the Master Brand Brief v0.5.2 §1.1 Cultural activation exactly as **Let us cultivate our city with data.**, write Thai from meaning and evidence rather than translated sentence structure, prefer an approved transparent logo directly on its governed surface, treat quiet as perceptual calm rather than a blank component, keep useful photography large, use governed gradient atmosphere to direct attention, prove adoption value on the same work object, use deterministic script-aware technical typography, separate contextual discovery jobs and publication states, preserve identity-context and Color Set/build parity, and remove connector lines that do not encode a real relationship;
...
4. this v0.8.9 master governs how those truths become native-language communication, visual hierarchy, composition, interaction, accessibility, responsive behavior, tokens, states, profiles, schemas, and QA; it MUST NOT invent or override brand/product truth outside its explicit amendments;
...
The v0.8.9 public-safe machine specification package MUST supply `build-card.schema.json`, `manifest.schema.json`, generated tokens, surface recipes, identity and voice records, rule ledgers, and preflight mappings before package-level v0.8.9 conformance is claimed. An artifact may report `machineValidation: passed` only after the package validator and every applicable artifact-level automated and manual gate run against that artifact. Package availability alone is not a pass.
...
- `radius-pill` is reserved for status, compact filters, tags, and the approved text-action family under `[CTRL-02]`; it is not generic section decoration and does not redefine the geometry of cards, fields, navigation rows, tabs, or segmented selectors.
...
Theme behavior follows the Build Card. Every new or materially changed interactive HTML artifact MUST support `dual`, default to **Auto/system**, and provide one accessible visible override. `static_export` is exempt. Any other fixed-theme interactive output requires a named, owned, expiring exception and cannot claim full v0.8.9 production conformance.
...
| `Button` | native button, intent label, approved action-family geometry, focus, busy/disabled, 44px target, wrapping/narrow-width behavior |
...
Within one interface, text-labelled primary and secondary actions SHOULD form one consistent action family using an approved radius token; the Landometer reference uses `radius-pill`. Mixed icon-plus-label actions stay in that capsule family with sufficient locale-aware inline padding. Labels may wrap without truncation, keep a minimum 44 px target, and stack or become full-width at narrow viewports. Icon-only utilities retain a 44 × 44 circular target.

Shape never replaces visible intent, hierarchy, focus, disabled/pending state, real effect, receipt, or recovery. Cards, form fields, disclosures, table rows, tooltips, navigation rows, tabs, and segmented selectors keep their own semantic geometry even when interactive; do not apply the action capsule to every clickable or rounded container. Product-specific component geometry remains product-owned and requires review before adopting the shared action family.
...
```md
Build this artifact with Landometer Design System v0.8.9.
...
<html
  lang="th"
  data-ds="landometer"
  data-ds-version="0.8.9"
  data-build-card-version="0.8.9"
...
## 11. v0.8.8-r3 → v0.8.9-r1 migration and earlier regression history
...
This regression fixture MUST fail v0.8.9 first-viewport, brand-specificity, control-necessity, token, font-delivery, mobile-wayfinding, native-language, identity-surface, and pre-AHA-sharing gates.

## 12. Definition of done `[DONE-01]`

An artifact is v0.8.9-ready when:
...
Gradient registry: `landometer-atmosphere-gradient-v2`. Color Set:
`color-srgb-02`. All delivery is exact SDR sRGB. The fifteen new values in the
...
The v0.8.9 Markdown remains the concise human front door. The v0.8.9 public-safe machine package MUST generate and validate the following machine-readable implementation projection before package-level v0.8.9 conformance is claimed:
...
Owner approval closes the v0.8.9 authoring decision; generated records close only package-level machine gates after regeneration. The active disposition is:

| Gate | Disposition |
|---|---|
| Manifest and tokens | Manifest 2.0 and Token Schema 6 structures remain unchanged; fifteen gradient-only sRGB values are authorized through Color Set `color-srgb-02` and require generated registry entries, hashes, and validation before machine-package conformance |
| Build Card | schema 0.8.9 carries quiet-region, atmosphere-surface, connector, value-proof, local-foreground, discovery-state/action, identity-context, and Color Set/build records |
...
| Native-Thai fixtures | `thai-voice-fixtures-v0.8.7-v1` remains the approved fixture set because v0.8.9 changes composition, not voice; a future fixture revision requires its own approval |
...
| Public and skill package alignment | the version-pinned authoring skill MAY ship with an explicit `machinePackage: pending` boundary; v0.8.9 public specification, generated skill payloads, release lock, Build Card/Manifest schemas, surface recipes, connector schema, fixtures, identity/media/font manifests, source fingerprint, tokens, adapters, and all migration/rule ledgers MUST be version-locked before machine-package release or package-level conformance |
| Color delivery | Color Set `color-srgb-02` appends fifteen gradient-only values while `color-srgb-01` remains immutable; generated delivery records must bind Color Set, artifact build, token/scale paths/versions/hashes, immutable artifacts, aliases/deprecations, explicit sRGB serialization, and exact QA/post-deploy evidence |
