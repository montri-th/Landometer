# Landometer Design System v0.8.9 — Implementation clarifications

**Status:** Accepted artifact application record; reconciled through 7 August 2026
**Authority:** Non-normative implementation evidence. The owner-approved v0.8.9-r1 authoring master governs; this note does not override it.
**Delivery state:** UI build `ui-20260807-07` is prepared locally but not yet published; the regenerated public-safe machine package remains pending.
**Applies to:** The v0.8.9 Design Identity Playground implementation
**Preserves:** `[CORE-03]`, `[CORE-10]`, `[CTRL-01]`, `[STATE-01]`, `[REFERENCE-01]`, `[DELIVERY-01]`, `[SHARE-01]`, `[CONTEXT-01]`, `[PUB-01]`, and `[AGENT-OUT-01]`

This note records how the current playground carries forward the approved v0.8.8 experience clarifications and complete `Let us cultivate our city with data.` Culture / Rally Cry, then applies the owner-approved v0.8.9 atmosphere-gradient family. Artifact-specific choices, URLs, assets, hashes, and fixture content remain local evidence here.

## IMPL-SURFACE-COLOR-01 — Preserve governed atmosphere and make color guidance findable

The Try fixture uses two teaching presets so each inspection lens opens with the intended visual character: Brand DNA resolves to Measure Deep in light mode, with its dark-blue directional gradient; Brand Voice resolves to Ground Mist, the muted light-blue gradient. Brand Visual remains the atmosphere playground and keeps the user-selected Measure, Ground, or Cultivate surface. Theme counterparts come from the same frozen registry rather than runtime color generation.
...
Keep each canonical gradient recipe intact. Repair readability through the surface-owned foreground contract:

1. choose the tested local foreground;
2. keep the standalone gradient visible when its foreground contract already passes; do not add a blanket scrim;
3. place dense metadata on the declared local translucent or opaque panel when that grouping improves scanning;
...
Color guidance is a first-class Implementation Library route. Start by naming the job—Brand, Interaction, Semantic, Atmosphere, Series, Scale, Map, or Depth—then choose among solids/states, gradients, sequential/diverging scales, categories, opacity/layers, and lines/strokes. The concise router leads to realistic fixtures first and the complete generated atlas second. The atmosphere section discloses all six tonal recipes plus the rare Diversity Spectrum, their exact stops, foreground pairs, minimum sampled contrast, theme defaults, and prohibited uses.

The artifact mints 15 exact six-digit sRGB values for gradient stops only. They are governed by `landometer-atmosphere-gradient-v2` and may not be reused as solid, data, state, map, interaction, product-identity, or logo colors. It does not generate analytical ramps at runtime or promote candidate stroke widths, dash conventions, or overlap formulas into normative tokens. Categorical capacity remains: color alone up to six; add stable shape or pattern for seven to ten; above ten, group, filter, use small multiples, or provide a table.

This resolves the v0.8.8 visual duplication where surface and motif recipes were nearly identical, while preserving product-neutral shared-layer boundaries. The teaching presets themselves remain artifact-specific.

## IMPL-COLOR-SET-PARITY-01 — Prove the same build before judging device color

A report that two devices show a different Color Set is first a delivery-parity question, not evidence of a mere display shift. This artifact binds color delivery to `assets/data/color-delivery.v0.8.9.json` and registry ID `color-srgb-02`.

Color identity and UI-build identity are separate. The Color Set remains `color-srgb-02` while each UI-only change mints a new append-only `artifactBuild` ID and immutable filename. A governed color, gradient, token-source, or scale-source change requires a new Color Set ID; a responsive, accessibility, copy, or release-tool change does not.
...
The expected post-publication immutable handoff URL is:

```text
https://montri-th.github.io/Landometer/landometer-design-system-v0.8.9-standalone.color-srgb-02.ui-20260807-07.html
```

Before publication, the local immutable artifact-build file is the visual-QA authority for the current UI build. The hosted URL becomes handoff authority only after exact-byte post-deploy verification. The earlier `landometer-design-system-v0.8.9-standalone.color-srgb-02.html` remains a byte-stable Color Set baseline and is not rewritten when UI code changes. `index.html` and `landometer-design-system-v0.8.9-standalone.html` are latest aliases. GitHub Pages currently serves latest aliases with `Cache-Control: max-age=600`, so two devices can temporarily receive different releases after publication. A latest-alias mismatch is classified as cache/build skew; do not describe it as color drift and do not use it as cross-device approval evidence.
...
This artifact applies the owner-approved Color Set/build parity contract. The 15 new values remain gradient-stop-only and do not alter `tokens.json` or `scales.json`; the carried v0.8.6 token/scale registries are not promoted to gated v0.8.9 package conformance.

## IMPL-FONT-DELIVERY-01 — Keep Thai font delivery deterministic in Safari and portable files

The hosted source and the portable standalone are separate delivery modes. The hosted source creates Bai Jamjuree 400 and IBM Plex Sans Thai 400 preloads only on `http:` or `https:`. It contains no declarative cross-origin font preload that Safari could attempt from a `file://` null origin.

Opening `deployment/index.html` directly from disk hands off to `landometer-design-system-v0.8.9-standalone.html` while preserving query parameters and the fragment. The standalone embeds all governed font faces, marks itself with `data-standalone="true"`, and disables both the handoff and hosted-only preload logic. This addresses an artifact delivery defect; it does not change the v0.8.9-r1 typography contract or remove its resilience fallbacks.

Font readiness is fail-closed. Representative glyph loads cover all nine active subsets: Thai Bai Jamjuree 400/600, Thai IBM Plex Sans Thai 400, Thai IBM Plex Sans Thai Looped 700, and the corresponding governed Latin companions across Bai Jamjuree 400/600, IBM Plex Sans Thai Looped 700, JetBrains Mono 400, and Arvo 700. Each load must return at least one loaded face and pass `document.fonts.check`. The artifact records `data-thai-body-ready`, `data-thai-technical-ready`, `data-thai-display-ready`, `data-latin-companions-ready`, `data-fonts-ready`, and `data-font-delivery`; delivery starts `pending`, rejected loads become `failed`, a five-second deadline becomes `timeout`, and a missing Font Loading API becomes `unavailable`. A computed fallback family is not accepted as successful delivery.

Safari direct-file verification remains a manual gate until the redirect, embedded font faces, representative Thai glyphs, and readiness attributes are observed on the supported Safari/device matrix.
...
This is the artifact delivery evidence for the owner-approved v0.8.9-r1 technical pair. The exact nine subset files, optical tuning, and review state remain artifact records; they do not certify the future public-safe machine package.
...
The v0.8.9 playground includes:
...
This is the current artifact application of the owner-approved semantic action geometry. It does not retroactively declare every rounded rectangle across Landometer a button or change product-owned component contracts without product review.
