...
**Release:** v0.8.8
**Authoring revision:** v0.8.8-r3 — owner-approved brand-line authority correction integrated 7 August 2026; experience clarifications integrated in r2 on 6 August 2026
**Manifest version:** 2.0 — unchanged
**Token schema version:** 6 — unchanged
**Build Card schema version:** 0.8.8
**Status:** Owner-approved simplified normative authoring master; the v0.8.8-r3 public-safe machine package, release lock, source fingerprint, rule ledger, schemas, recipes, and preflight MUST be regenerated before package-level v0.8.8 conformance is claimed
**Prepared:** 25 July 2026; experience clarifications approved and integrated 6 August 2026; brand-line authority corrected 7 August 2026
...
### 1.0A Canonical record shapes for v0.8.8-r2 arrays
...
v0.8.8 changes composition authority and applicable QA. The owner-approved v0.8.8-r2 authoring amendment also governs technical typography, publication/discovery state evidence, cross-context identity delivery, action-capsule geometry, local foreground contracts, Context Discovery actions, and Color Set/artifact-build parity. Authoring revision v0.8.8-r3 corrects one authority conflict by restoring the exact Cultural activation line from Master Brand Brief v0.5.2 §1.1: **“Let us cultivate our city with data.”** It does not change Manifest 2.0, Token Schema 6, raw color values, Profiles, Trigger Packs, optional capabilities, the other protected brand lines, product/evidence boundaries, or the default visible route.
...
Opacity never carries critical meaning alone. Named depth roles remain `depth.base`, `depth.context`, `depth.data`, `depth.focus`, `depth.action`, and `depth.memory`; they organize canvas → context → evidence → selection → action → receipt/history without creating local opacity or z-index values. v0.8.8 defines no fixed depth-role-to-opacity-role mapping; any such mapping remains `candidate` until separately approved and generated.
...
- Build Card 0.8.8 adds records for value proof, local foreground contracts, discovery states/actions, identity-delivery contexts, and Color Set/build parity in addition to quiet regions, gradient surfaces, and connectors; Manifest 2.0 and Token Schema 6 remain unchanged;
...
- no Profile, Trigger Pack, optional capability, raw color value, additional protected brand line, product truth, evidence boundary, menu, mode, or default-route scene is added; r3 corrects the Cultural activation text to match the governing Master Brand Brief.

## C5. Reconciliation record

This release integrates three owner-approved inputs into the v0.8.7 simplified integrated master: (1) the Perceptual Quiet, Brand Atmosphere Surface, Full-Bleed Media, and Connector Integrity composition patch; (2) the v0.8.8 experience-clarifications amendment recorded in `normative-patches/landometer-design-system-v0.8.8-experience-clarifications.approval.yml`; and (3) the Master Brand Brief v0.5.2 §1.1 Cultural activation correction recorded in `normative-patches/landometer-design-system-v0.8.8-brand-line-authority-correction.approval.yml`. None of these records is required as a runtime sidecar because every approved amendment now lives in its owning rule, Build Card record, QA gate, migration section, or Appendix A recipe. The approval records prove provenance and scope; they are not competing normative authorities.
...
The owner instructions dated 25 July and 6–7 August 2026 approve integration of the recorded patches and brand-authority correction into this v0.8.8 authoring master. The 6 August proposal remains immutable reconciliation evidence; the 7 August decision receipt records the Brand Brief reconciliation. Neither is a runtime sidecar or competing authority. Existing identity, media, font, and Thai fixture approvals remain hash- and scope-specific; they do not automatically approve a new crop, gradient-over-photo rendition, dark/busy logo pairing, cross-context identity asset, third-party right, unreviewed visible screen, product claim, deployment, or implementation that has not run its own v0.8.8 artifact QA.
...
| Build Card | schema 0.8.8 carries quiet-region, atmosphere-surface, connector, value-proof, local-foreground, discovery-state/action, identity-context, and Color Set/build records |
...
| Public and skill package alignment | the version-pinned authoring skill MAY ship with an explicit `machinePackage: pending` boundary; v0.8.8 public specification, generated skill payloads, release lock, Build Card/Manifest schemas, surface recipes, connector schema, fixtures, identity/media/font manifests, source fingerprint, tokens, adapters, and all migration/rule ledgers MUST be version-locked before machine-package release or package-level conformance |
| Color delivery | no raw color is added; generated delivery records must bind Color Set, artifact build, token/scale paths/versions/hashes, immutable artifacts, aliases, sRGB serialization, and exact QA/post-deploy evidence |
