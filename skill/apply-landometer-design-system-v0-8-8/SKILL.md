---
name: apply-landometer-design-system-v0-8-8
description: Apply or audit the owner-approved Landometer Design System v0.8.8 human-readable authoring master, including the r2 experience clarifications and the r3 protected brand-line authority correction, in local design, content, presentation, data, and interface artifacts. Use when the user explicitly requests v0.8.8 or invokes this skill; require Build Card 0.8.8 and exactly one profile, preserve shared-versus-product evidence boundaries, and report package and artifact gates honestly. Do not claim generated machine-package conformance while its schemas, manifests, recipes, fixtures, hashes, rule ledgers, and preflight remain pending; do not publish or create external effects.
---

# Apply Landometer Design System v0.8.8
...
3. Read [references/landometer-design-system-v0.8.8-authoring-master.md](references/landometer-design-system-v0.8.8-authoring-master.md) completely. Do not substitute memory, v0.8.6, v0.8.7, a webpage, or the proposal sidecar.
4. Accept only `dsVersion: 0.8.8`, Build Card `0.8.8`, Manifest `2.0`, and Token Schema `6`. Stop on another version rather than silently upgrading or downgrading.
...
## Apply the approved v0.8.8-r3 authoring authority
...
Do not invent or present as v0.8.8:
...
4. Use exact Appendix A authoring values. Do not copy the carried v0.8.6 token/scale files and relabel them v0.8.8.
...
6. the explicit statement that generated v0.8.8 machine-package conformance remains unclaimed until the package listed in the release lock is regenerated and validated.
